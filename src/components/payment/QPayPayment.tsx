...
import type { Cargo, Payment, PaymentStatus, PAYMENT_STATUS_LABELS, PAYMENT_STATUS_COLORS } from '@/types/cargo';
...
export default function QPayPayment({ cargos, totalAmount, onPaymentComplete }: QPayPaymentProps) {
  ...
  useEffect(() => {
    ...
    const checkExistingPayment = async () => {
      ...
      // Check if any cargo already has a payment
  ...
  const createInvoice = async () => {
    ...
    if (cargos.length === 0 || totalAmount <= 0) {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/qpay-create-invoice`, {
        ...
        body: JSON.stringify({
          amount: totalAmount,
          cargo_ids: cargos.map(c => c.id),
          description: `OnlyCargo - ${cargos.length} ачаа төлбөр`,
        }),
      });
    ...
    try {
      ...
      if (updatedPayment) {
        ...
        if (updatedPayment.status === 'paid') {
          ...
          onPaymentComplete?.(updatedPayment as Payment);
          setIsOpen(false);
  ...
  const renderPaymentStatusLabels = (status: PaymentStatus) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button disabled={cargos.length === 0 || totalAmount <= 0}>
          <CreditCard className="mr-2 h-4 w-4" />
          QPay төлбөр ({totalAmount.toLocaleString()}₮)
        </Button>
      </DialogTrigger>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <Badge variant={payment.status === 'paid' ? 'default' : 'secondary'}>
                  {renderPaymentStatusLabels(payment.status as PaymentStatus)}
                </Badge>
              </div>
