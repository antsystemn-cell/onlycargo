...
export default function KoreaAddress() {
  ...
  const effectiveSelectedId =
    selectedAddressId || (koreaWarehouseAddresses.length === 1 ? koreaWarehouseAddresses[0].id : '');
  const selectedAddress = koreaWarehouseAddresses.find((a) => a.id === effectiveSelectedId);
  const userPhone = profile?.phone || '[Таны утасны дугаар]';

  const fullAddress = selectedAddress
    ? `받는사람: ${selectedAddress.receiver}\n전화번호: ${selectedAddress.phone}\n지역: ${selectedAddress.region}\n상세주소: ${selectedAddress.address} (${selectedAddress.prefix}-${userPhone})`
    : '';
    ...
    <div className="flex flex-col">
      <header className="sticky top-0 z-40 border-b bg-card px-4 py-3">
        <div className="mx-auto flex max-w-md items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Солонгос агуулахын хаяг</h1>
        </div>
      </header>

      <main className="flex-1 px-4 py-6">
        <div className="mx-auto max-w-md space-y-4">
          {koreaWarehouseAddresses.length > 1 && (
            <Card>
              <CardContent className="pt-4 space-y-2">
                ...
                <Select value={effectiveSelectedId} onValueChange={setSelectedAddressId}>
                  ...
                  <SelectContent>
                    {koreaWarehouseAddresses.map((addr) => (
          ...
          >
            <CardHeader>
              ...
              <CardDescription>
                Coupang, Gmarket, 11st зэрэг Солонгосын онлайн дэлгүүрүүдээс захиалахдаа энэ хаягийг ашиглана
              </CardDescription>
            </CardHeader>
              ...
              {selectedAddress && (
                <>
                  <div className="space-y-3 rounded-lg bg-muted/50 p-4 font-mono text-sm">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">받는사람 (Хүлээн авагч):</p>
                      <p className="font-medium">{selectedAddress.receiver}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">전화번호 (Утас):</p>
                      <p className="font-medium">{selectedAddress.phone}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">지역 (Бүс нутаг):</p>
                      <p className="font-medium">{selectedAddress.region}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">상세주소 (Дэлгэрэнгүй хаяг):</p>
          ...
          <Card>
            ...
            <CardContent>
              <ol className="space-y-2 text-sm text-muted-foreground list-decimal list-inside">
                <li>Coupang, Gmarket, 11st зэрэг сайтаас захиалга өгөхдөө дээрх хаягийг оруулна</li>
