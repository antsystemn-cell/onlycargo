...
import { Loader2, RefreshCw, Navigation, Save } from 'lucide-react';
...
export default function MongoliaDeliveryManagement() {
  ...
  const [editing, setEditing] = useState<Record<string, Partial<DeliveryOrder>>>({});
  ...
  const fetchOrders = async () => {
    ...
    const list = (data || []) as DeliveryOrder[];
    setOrders(list);
  ...
  const saveConfig = async () => {
    ...
    const { error: e1 } = await supabase.from('site_settings').upsert(
      { key: 'mongolia_delivery_enabled', value: enabled as any },
      { onConflict: 'key' }
    );
    const { error: e2 } = await supabase.from('site_settings').upsert(
      { key: 'mongolia_delivery_fee', value: fee as any },
      { onConflict: 'key' }
    );
  ...
  const patchOrder = (id: string, patch: Partial<DeliveryOrder>) => {
    setEditing((prev) => ({ ...prev, [id]: { ...prev[id], ...patch } }));
  };

  const saveOrder = async (order: DeliveryOrder) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Монгол дахь хүргэлт</h1>
          <p className="text-sm text-muted-foreground">Монгол дотор хот хоорондын хүргэлтийн захиалга</p>
        </div>
      </div>
              ...
              <Card key={o.id}>
                <CardContent className="space-y-3 p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      <Label className="text-xs">Хүргэлтийн үнэ (₮)</Label>
                      <Input
                        type="number"
                        min="0"
                        step="1000"
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <Textarea
                        ...
                        placeholder="Хугацаа, нөхцөл, тодруулга..."
                      />
                    </div>
                  </div>
